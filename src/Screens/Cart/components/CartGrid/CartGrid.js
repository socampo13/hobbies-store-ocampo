...
export const CartGrid = () => {
    ...
    const addNewOrder = (buyer) => {
      ...
      const orders = dataBase.collection("forms");
      try {
    ...
    <div className={classes.container}>
      <Button className={classes.buttonDelete} onClick={clear}>Borrar carrito</Button>
            ...
            <div className={classes.container}>
              {
                showForm ? <Formulario addOrder={addOrderUpdateItems}/> : 
                <div className={classes.messageOrder}>
                  <Typography variant="h3">¡Gracias por comprar con nosotros!</Typography>
                  <Typography>Pedido número: {orderId}</Typography>
                </div>
              }
            </div>
